...
def call_result1(label_result1,n1, n2):
    num1 = (n1.get())
    num2 = (n2.get())
    hour = 10**(1.998-1.536*math.log10(float(num1)/float(num2))) 
    label_result1.config(text="Result is %0.2f (hr)" % hour)

def call_result2(label_result2,n1):
    num1 = (n1.get())
    need = float(num1)/(10**((1.998-math.log10(24))/1.536))
...
buttonCal1 = tk.Button(root, text="starting time of leaf browning", command=call_result1).grid(row=4, column=0)
buttonCal2 = tk.Button(root, text="number of watermelon", command=call_result2).grid(row=5, column=0)
